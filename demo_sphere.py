...
ASSET_DIR ="/home/abc/isaacsim_assets/Assets/isaac/4.5/Isaac/Robots/Factory"
USD_PATHS = {
    "box": "assets/box.usd",
    "bolt": "assets/bolt.usda",
    "light": "assets/lights.usda",
    "sphere":"assets/xiaoqiu/sphere_small/sphere_small.usd",
    "cube" : "assets/sphere_small.usd",
    ...
    def get_scene_cfg(self):
        ...
        class DigitSceneCfg(InteractiveSceneCfg):
            ...
            cube = AssetBaseCfg(
                ...
                spawn=sim_utils.UsdFileCfg(
                    # usd_path=USD_PATHS["cube"],
                    # usd_path="/home/abc/TactSim-IsaacLab_4_5/assets/cube_small/cube_small.usd",
                    usd_path="/home/abc/TactSim-IsaacLab_4_5/assets/sphere_small/sphere_small.usd",
                    rigid_props=sim_utils.RigidBodyPropertiesCfg(rigid_body_enabled=False), scale=(0.9,0.9,0.9)
                    ),
...
#深度           
            depth_r = scene["camera_r"].data.output["distance_to_image_plane"][0].detach().cpu().numpy()
            depth_l = scene["camera_l"].data.output["distance_to_image_plane"][0].detach().cpu().numpy()
            # print(depth_l.shape)
            depth_r[self.boder_ind[:, 0], self.boder_ind[:, 1]] = 0
            depth_l[self.boder_ind[:, 0], self.boder_ind[:, 1]] = 0
                      # 设置有效深度范围

            depth_r = cv2.normalize(depth_r, None, 0, 70000, cv2.NORM_MINMAX)
            depth_r = depth_r.astype(np.uint8)
            depth_l = cv2.normalize(depth_l, None, 0, 70000, cv2.NORM_MINMAX)
            depth_l = depth_l.astype(np.uint8)
            # depth_r = cv2.normalize(depth_r, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
            # depth_l = cv2.normalize(depth_l, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
            depth_concat = np.hstack((depth_l,depth_r))
            
            cv2.imshow("DEPTH Image", depth_concat)
...
# RGB 
            ...
            if count % 450 == 0:
                ...
                # Gripper sequence
                GRIPPER_CLOSE_VALUE = 0.001  # 校准后的闭合位置
                GRIPPER_OPEN_VALUE = 0.001  # 校准后的张开位置
                if count < 100:
                    gripper_close = torch.tensor([-GRIPPER_CLOSE_VALUE,GRIPPER_OPEN_VALUE], device=sim.device).unsqueeze(0).repeat(scene.num_envs, 1)
